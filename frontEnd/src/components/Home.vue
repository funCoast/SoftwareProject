...
export default {
  ...
  computed: {
    currentAgents() {
      switch (this.currentAgentTab) {
        case 'hot':
          return this.hotAgents
        case 'following':
          return this.followingAgents
        case 'favorite':
          return this.favoriteAgents
        default:
          return this.hotAgents
      }
    },
    totalPages() {
      return Math.ceil(this.currentAgents.length / this.itemsPerPage)
    },
    paginatedAgents() {
      const start = (this.currentPage - 1) * this.itemsPerPage
      const end = start + this.itemsPerPage
      return this.currentAgents.slice(start, end)
    }
  },
  watch: {
    currentAgentTab() {
