<template>
  <div class="plugin-container">
    ...
    <div class="form-group">
      <label>源时区</label>
      <select v-model="config.sourceTimezone" class="form-control">
    ...
    <div class="form-group">
      <label>目标时区</label>
      <select v-model="config.targetTimezone" class="form-control">
        <option value="UTC">UTC</option>
        <option value="Asia/Shanghai">Asia/Shanghai</option>
        <option value="America/New_York">America/New_York</option>
        <option value="Europe/London">Europe/London</option>
...
interface TimezoneConfig extends TimePluginConfig {
...
const config = ref<TimezoneConfig>({
...
async function run(): Promise<PluginResult> {
  try {
    ...
    const time = dayjs.tz(config.value.date, config.value.sourceTimezone)
    const convertedTime = time.tz(config.value.targetTimezone).format(config.value.format)
    ...
    const errorResult = {
      success: false,
      data: null,
      error: error instanceof Error ? error.message : '时区转换失败'
    }
